...
impl TuiApp {
    ...
    fn draw_metrics(&self, f: &mut ratatui::Frame, area: Rect) {
        ...
        let avg_response = Paragraph::new(format!("Avg Response: {:.2}ms", metrics.avg_response_time))
            .style(Style::default().fg(Color::Blue))
            .block(Block::default().borders(Borders::ALL));
        f.render_widget(avg_response, left_chunks[2]);
        
        let success_rate = if metrics.requests_sent > 0 {
            (metrics.requests_completed - metrics.requests_failed) as f64 / metrics.requests_sent as f64 * 100.0
        } else {
            0.0
        };
        ...
        let bytes_received = Paragraph::new(format!("Bytes: {}", format_bytes(metrics.bytes_received)))
            .style(Style::default().fg(Color::Cyan))
            .block(Block::default().borders(Borders::ALL));
        ...
        let status_items: Vec<ListItem> = metrics.status_codes
            .iter()
            .map(|(code, count)| {
        ...
        if !metrics.errors.is_empty() {
            let error_items: Vec<ListItem> = metrics.errors
                .iter()
                .take(5)
                .map(|(error, count)| {
