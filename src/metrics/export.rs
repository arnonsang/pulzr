...
impl CsvExporter {
    ...
    pub async fn export_summary<P: AsRef<Path>>(&self, path: P) -> Result<()> {
        ...
        writer.write_record(&["successful_requests", &summary.successful_requests.to_string()])?;
        writer.write_record(&["failed_requests", &summary.failed_requests.to_string()])?;
        writer.write_record(&["test_duration_secs", &format!("{:.2}", summary.test_duration_secs)])?;
        writer.write_record(&["avg_rps", &format!("{:.2}", summary.avg_rps)])?;
        writer.write_record(&["avg_response_time", &format!("{:.2}", summary.avg_response_time)])?;
        ...
        writer.write_record(&["total_bytes_received", &summary.total_bytes_received.to_string()])?;
    ...
    pub fn print_summary(&self, summary: &FinalSummary) {
        ...
        println!("Total Bytes: {}", format_bytes(summary.total_bytes_received));
