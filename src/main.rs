...
use clap::Parser;
use pulzr::{
    auth::{AuthMethod, JwtConfig, JwtManager, ApiKeyConfig, ApiKeyManager},
    cli::Cli,
    client::HttpClient,
    debug::DebugConfig,
    ...
    websocket::{TestConfig, WebSocketServer, WebSocketMessage},
...
fn create_http2_config(cli: &Cli) -> Result<Http2Config> {
    ...
    if cli.http2 && cli.http1_only {
        return Err(anyhow::anyhow!("Cannot specify both --http2 and --http1-only"));
    }
...
async fn main() -> Result<()> {
    ...
    if cli.memory_optimize && cli.url.is_none() && cli.scenario.is_none() && cli.endpoints.is_none() {
        ...
        println!("Current memory profile: {}", cli.memory_profile.as_deref().unwrap_or("default"));
    ...
    if cli.list_dashboards || cli.dashboard_info.is_some() || cli.dashboard_import.is_some() || cli.dashboard_validate.is_some() {
            ...
            match grafana_manager.list_dashboards() {
                Ok(dashboards) => {
                    if dashboards.is_empty() {
                        println!("   No dashboards found in directory: {}", cli.dashboards_dir.as_deref().unwrap_or("dashboards"));
                    } else {
    ...
    let client = Arc::new(HttpClient::new(
        cli.url.clone().unwrap_or_else(|| "http://placeholder.com".to_string()),
    ...
    let (actual_websocket_port, actual_webui_port) = if let Some(mut ws_server) = websocket_server {
        ...
        let config = TestConfig {
            url: cli.url.clone().unwrap_or_else(|| "Scenario Mode".to_string()),
    ...
    let tui_handle = if !cli.no_tui && !cli.webui {
        let mut tui_app = TuiApp::new(Arc::clone(&stats_collector))
            .with_quit_sender(quit_sender.clone());
        Some(tokio::spawn(async move {
    ...
    if let Some(url) = &cli.url {
        ...
        println!("Endpoints: {} ({} endpoints)", endpoints.name, endpoints.endpoints.len());
        for endpoint in &endpoints.endpoints {
            println!("  - {}: {} {}", endpoint.name, endpoint.get_method(&endpoints.defaults), endpoint.url);
        }
    }
    println!("Method: {:?}", cli.method);
    if let Some(ramp_duration) = cli.ramp_up {
        println!("Ramp-up: {} pattern over {}s to {} concurrent", 
    ...
    println!("User-Agent mode: {}", if cli.random_ua {
        format!("Random ({} agents)", user_agent_manager.get_agent_count())
    } else if cli.user_agent.is_some() {
        "Custom".to_string()
    } else {
    ...
    if cli.debug {
        println!("Debug mode: Level {} ({})", cli.debug_level, match cli.debug_level {
    ...
    if cli.prometheus {
        println!("📊 Prometheus metrics available at: http://127.0.0.1:{}/metrics", cli.prometheus_port);
    }
