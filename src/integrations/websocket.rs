...
impl WebSocketServer {
    ...
    pub async fn start(&mut self) -> Result<u16> {
        let actual_port = find_available_port(self.port, 50)
            .ok_or_else(|| anyhow::anyhow!("Could not find available port starting from {}", self.port))?;
        ...
        if actual_port != self.port {
            println!("WebSocket server listening on: ws://{} (auto-selected from preferred port {})", addr, self.port);
        } else {
