...
impl StatsCollector {
    ...
    pub fn with_websocket_sender(mut self, sender: broadcast::Sender<crate::websocket::WebSocketMessage>) -> Self {
        self.request_log_sender = Some(sender);
        self
    }
    
    pub fn clone_with_websocket_sender(&self, sender: broadcast::Sender<crate::websocket::WebSocketMessage>) -> Self {
        Self {
    ...
    async fn check_alerts(&self) {
        ...
        let window_start = now - chrono::Duration::seconds(self.alert_config.alert_window_seconds as i64);
        ...
        let recent_errors = recent_results.iter()
        ...
        if error_rate > self.alert_config.error_rate_threshold {
            ...
            if !active_alerts.iter().any(|a| matches!(a.alert_type, AlertType::ErrorRate)) {
    ...
    async fn check_performance_degradation(&self, results: &[RequestResult], now: DateTime<Utc>) {
        ...
        let baseline_start = now - chrono::Duration::seconds(self.alert_config.baseline_window_seconds as i64);
        let baseline_end = now - chrono::Duration::seconds(self.alert_config.alert_window_seconds as i64);
        ...
        let recent_start = now - chrono::Duration::seconds(self.alert_config.alert_window_seconds as i64);
        let recent_results: Vec<_> = results.iter()
        ...
        let baseline_avg = baseline_results.iter()
            .map(|r| r.duration_ms)
            .sum::<u64>() as f64 / baseline_results.len() as f64;
        
        let recent_avg = recent_results.iter()
            .map(|r| r.duration_ms)
            .sum::<u64>() as f64 / recent_results.len() as f64;
        ...
        if increase_percentage > self.alert_config.degradation_threshold {
            ...
            // Check if we already have this alert active
            let mut active_alerts = self.active_alerts.write().await;
            if !active_alerts.iter().any(|a| matches!(a.alert_type, AlertType::PerformanceDegradation)) {
                active_alerts.push(alert.clone());
...
mod tests {
    ...
    fn test_alert_type_equality() {
        ...
        assert_eq!(AlertType::Custom("test".to_string()), AlertType::Custom("test".to_string()));
